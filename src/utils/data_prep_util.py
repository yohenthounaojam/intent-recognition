...
def get_all_frames_labels_all_vids(session_ids_lst, layer, fps):
  ...
  for session_id in session_ids_lst:
    ...
    save_path = os.path.join(HDD_ALL_FRAMES_ALL_VIDS_LABELS_PATH,"session_%s_layer_%s.npy" % (session_id, layer))
    # np.save(HDD_ALL_FRAMES_ALL_VIDS_LABELS_PATH + "/session_%s_layer_%s.npy" % (session_id, layer), session_labels)
    np.save(save_path, session_labels)
